...
public class IdealArray_2338 {
    ...
    private int backtrackCount(int n, int maxValue) {
    ...
    private int count(
            ...
            int n,
            int[][] memo) {
        if(index == n - 1 && val <= maxValue) return 1;
        if(val > maxValue) return 0;
        if(memo[index][val] != -1) return memo[index][val];
        int res = 0;
        for(int i = val; i <= maxValue; i++){
            if(i % val == 0){
                res += count(i, index + 1, maxValue, n, memo);
                res %= MOD;
            }
        }
        memo[index][val] = res;
    ...
    public static void main(String[] args) {
        int n = 5, maxValue = 3;
        IdealArray_2338 sol = new IdealArray_2338();
        System.out.println(sol.idealArrays(n, maxValue));
    }
}
